
package ie.lyit.ccr.util;

/**
 *
 * @author juarezjunior
 */
public class CcrConstants {
	
	
	//JSF PAGES
	public static final String MAIN = "main.xhtml";
	public static final String ADD_PET = "addPet.xhtml";
	public static final String MY_PETS = "myPets.xhtml";
	public static final String ADD_PET_SHOP = "addPetShop.xhtml";
	public static final String SEARCH_RESULTS = "searchResults.xhtml";
	public static final String EDIT_USER = "editUser.xhtml";
	public static final String ADD_VETERINARY = "addVeterinary.xhtml";
	
	
	public static final String RESOURCE_BUNDLE_BASE_NAME = "/messages";	    
    public static final String MONGO_CLIENT = "MONGO_CLIENT";
    public static final String MORPHIA = "MORPHIA";
    public static final String MORPHIA_DATA_STORE = "MORPHIA_DS";
    public static final String MONGO_DB_FILE_DS = "pethelp-ds.xml";
    public static final String COMPASS_PATH_WINDOWS = "C:\\compass\\";
    public static final String COMPASS_PATH_LINUX = "/home/pethelpuser/compass/";    
    public static final String COMPASS_CONNECTION = "COMPASS";
    public static final String OP_TARGET = "opTarget";
    public static final String OP_SEARCH_KEY = "opSK";
    public static final String USER_ROLE = "user";
    public static final String ADMINISTRATOR_ROLE = "administrator";
    public static final String BUSINESS_ROLE = "business";
    public static final String SHOW_WELCOME_NOTIFICATION_BAR = "showWelcomeNotificationBar";
    
}
